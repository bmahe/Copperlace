...
Python structured rendering returns native Python `dict`, `list`, `str`,
numeric, `bool`, and `None` values. The wrapper requests compact JSON from the
C ABI internally and parses it before returning. Inferred rendering returns
text for string-valued and list-valued rules, or a formatted JSON string for
object-valued rules.

These wrapper values follow the Rust `CopperlaceValue` contract: structured
objects, arrays, strings, numbers, booleans, nulls, and large unsigned integers
are preserved through the native JSON bridge.
...
Java structured rendering returns JSON strings to avoid adding a JSON
dependency. No-argument structured JSON overloads return formatted JSON using
tabs for indentation. The `formatJson` parameter controls serialization:
`false` returns compact JSON and `true` returns formatted JSON.
