...
C ABI internally and parses it before returning. JSON-string helper APIs are not
exported in the Python wrapper.
...
dependency. The `formatJson` parameter controls serialization: `false` returns
compact JSON and `true` returns formatted JSON using tabs for indentation.
