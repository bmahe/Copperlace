...
`titlecase`, `article`, `past_tense`, and `pluralize`. The `article` processor
prefixes rendered text with `a` or `an`, as in `{item | article}`. The
`past_tense` processor converts one verb token to past tense, as in
`{action | past_tense}`. The `pluralize` processor converts one noun token to
plural form, as in `{creature | pluralize}`. Rust callers can register custom
