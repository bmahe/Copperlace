...
`possessive`. The `article` processor prefixes rendered text with `a` or `an`,
as in `{item | article}`. The `past_tense` processor converts one verb token to
past tense, as in `{action | past_tense}`. The `pluralize` and `singularize`
processors convert one noun token between singular and plural forms, and
`possessive` adds an English possessive suffix. Rust callers can register custom
