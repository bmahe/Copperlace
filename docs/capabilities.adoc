...
`titlecase`, `article`, and `past_tense`. The `article` processor prefixes
rendered text with `a` or `an`, as in `{item | article}`. The `past_tense`
processor converts one verb token to past tense, as in `{action | past_tense}`.
