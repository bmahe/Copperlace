...
public final class RuleSet implements AutoCloseable {
	 ...
	 * @throws CopperlaceException      if the config cannot be parsed or compiled
	 */
	public static RuleSet fromStringWithProcessors(final String config,
	 ...
	 * @throws CopperlaceException  if the file cannot be loaded, parsed, or compiled
	 */
	public static RuleSet fromFileWithProcessors(final Path path, final Map<String, CopperlaceProcessor> processors) {
		Objects.requireNonNull(path, "path");
		validateProcessors(processors);
	...
	public String renderStructuredJson(final String rule, final boolean formatJson, final int maxRecursionDepth) {
		...
		return NativeLibrary.INSTANCE
				.renderStructuredJsonWithContext(handle, rule, Map.of(), formatJson, maxRecursionDepth);
	}
	...
	public String renderStructuredJson(final String rule, final Map<String, String> context, final boolean formatJson,
		...
		validateContext(context);
		return NativeLibrary.INSTANCE
				.renderStructuredJsonWithContext(handle, rule, context, formatJson, maxRecursionDepth);
	}
